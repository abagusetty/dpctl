...
### Added
* In-order queue performance/interop primitives built on SYCL extensions, each feature-test-macro gated with a fallback to current behavior: eventless deferred USM free, eventless synchronous `memcpy`/`memset`/`prefetch` (`sycl_ext_oneapi_enqueue_functions`, via `DPCTLQueue_MemcpyEventless` / `DPCTLQueue_MemsetEventless` / `DPCTLQueue_PrefetchEventless`, used by `SyclQueue.memcpy` / `SyclQueue.prefetch` and the `dpctl.memory` copy/`memset` paths), and opt-in eventless kernel submission via `SyclQueue.submit_async(..., eventless=True)` (returns `None`; `DPCTLQueue_SubmitRangeEventless` / `DPCTLQueue_SubmitNDRangeEventless`); `dpctl::utils::keep_args_alive_in_order` which skips redundant keep-alive for same-queue in-order USM args; `SyclQueue.get_last_event()` / `SyclQueue.set_external_event()` (`sycl_ext_oneapi_in_order_queue_events`); and a non-blocking completion query `SyclQueue.empty()` (`sycl_ext_oneapi_queue_empty`, via `DPCTLQueue_Empty`; the SYCL analog of CuPy's `Stream.done`, complementing the eventless style). See `docs/in_order_optimization_notes.md`. Requires rebuilding `dpctl`.
...
* **Backward-incompatible (fork-local):** `dpctl.SyclQueue` is now **in-order by default**, while both in-order and out-of-order queues remain supported. Pass `property="out_of_order"` (or the raw-integer escape hatch `property=0`) to construct an out-of-order queue. The in-order fast paths are used when the queue is in-order and the out-of-order behavior is preserved otherwise: the order manager returns the no-op manager for in-order queues and the event-tracking `_SequentialOrderManager` for out-of-order queues; USM frees are deferred behind a host task only for in-order queues (eager otherwise); and synchronous `memcpy`/`memset`/`prefetch` use the eventless submit + queue-wait path on in-order queues and a precise per-event wait otherwise. Requires rebuilding `dpctl`.
