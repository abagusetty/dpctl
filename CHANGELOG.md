...
* **Backward-incompatible (fork-local):** `dpctl.SyclQueue` is now **always in-order**; out-of-order queues are no longer supported and cannot be created (the raw-integer escape hatch has been removed, and `SyclQueue.is_in_order` always returns `True`). All out-of-order-specific handling is removed accordingly: the order manager always returns a no-op manager (the `_SequentialOrderManager` and the C++ sequential order keeper are removed), USM frees are always deferred behind a host task, and synchronous `memcpy`/`memset` always use the eventless submit + queue-wait path. Requires rebuilding `dpctl`.
