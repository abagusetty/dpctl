...
// Release the USM allocation managed by ``opaque_ptr``. dpctl queues are
// in-order, so the release is ordered against work already submitted to the
// queue: a host task holding a copy of the managing ``shared_ptr`` is
// submitted, so the allocation is freed only when that host task runs, i.e.
// after all previously submitted work (including kernels enqueued by external
// libraries that share the queue) has completed. This avoids releasing memory
