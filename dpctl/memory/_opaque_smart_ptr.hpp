...
// that work, so the allocation is released eagerly instead -- matching the
// default behavior, where lifetime on out-of-order queues is managed by the
// caller through explicit event dependencies.
//
